...
public class DateUtil {
    ...
    /**
     *@description 获得指定前后的月份 集合
     *@author qicheng.qing selectMonth
     *@create  16/12/14 13:05
    */
    public static ArrayList<MonthBean> getArangeMonths(int selectYear, int selectMonth, int startMonth, int endMonth){

        ArrayList<MonthBean> result = new ArrayList<MonthBean>();


        Calendar min = Calendar.getInstance();
//        min.set(Calendar.YEAR,selectYear);

        Calendar max = Calendar.getInstance();

        TLog.error("++++++++++selectMonth:"+selectMonth);
        TLog.error("++++++++++selectYear:"+selectYear);

        min.add(Calendar.MONTH,-startMonth);
        max.add(Calendar.MONTH,endMonth);
        ...
        while (curr.before(max)) {
            MonthBean bean=new MonthBean(curr.get(Calendar.YEAR),curr.get(Calendar.MONTH));
            if (bean.getMonth()==0){//1月份开始后加当年年份
                MonthBean yearBean=new MonthBean();
                yearBean.setYear(bean.getYear());
                yearBean.setMonth(-1);
                yearBean.setCurrentMonth(-1);
                result.add(yearBean);

            }
            if (selectMonth==bean.getMonth()&&selectYear==bean.getYear()){
                TLog.error("---month:"+bean.getYear()+"-"+bean.getMonth());
                TLog.error("---month selectMonth:"+selectMonth);
                TLog.error("---month selectYear:"+selectYear);
            }else{
                bean.setCurrentMonth(-1);
                result.add(bean);
            }
